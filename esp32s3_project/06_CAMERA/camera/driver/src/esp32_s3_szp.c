...
esp_err_t bsp_display_new(void)
{
    ...
    ESP_GOTO_ON_ERROR(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)BSP_LCD_SPI_NUM, &io_config, &IO_handle), err, TAG,
                      "New panel IO failed");
...
static void task_process_LCD_handler(void *arg)
{
...
static void task_process_camera_handler(void *arg)
{
    while (true)
    {
...
void app_camera2LCD(void)
{

    xQueueLCDFrame = xQueueCreate(2, sizeof(camera_fb_t *));
    xTaskCreatePinnedToCore(task_process_LCD_handler, TAG, 3 * 1024, NULL, 5, NULL, 1);
    xTaskCreatePinnedToCore(task_process_camera_handler, TAG, 4 * 1024, NULL, 5, NULL, 0);
}
